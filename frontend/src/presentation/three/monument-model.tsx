...
const buildShape = (kind: MonumentShape, widthM: number, heightM: number) => {
  switch (kind) {
    ...
    case 'cross-top': return buildCrossTopShape(widthM, heightM);
    case 'curvy': return buildCurvyShape(widthM, heightM);
...
export const MonumentModel = ({
  ...
  const bodyHeight = useMemo(() => {
    switch (shapeKind) {
      ...
      case 'cross-top': return heightM * 0.86;
      ...
      case 'curvy': return heightM * 0.88;
      case 'classic':
          ...
          <group key={`stela-${stelaIdx}`} position={[offsetX, headstoneBaseY, -thicknessM / 2]}>
            ...
            <Suspense fallback={null}>
              {txt.inscriptionTrimmed && (
                <Text {...commonTextProps} position={[0, txt.headerY, textZ]} fontSize={txt.headerSize} outlineWidth={txt.headerSize * 0.11} lineHeight={TEXT_LINE_HEIGHT}>
                  {txt.inscriptionTrimmed}
                </Text>
              )}
              {txt.nameTrimmed && (
                <Text {...commonTextProps} position={[0, txt.nameY, textZ]} fontSize={txt.nameSize} outlineWidth={txt.nameSize * 0.12} lineHeight={TEXT_LINE_HEIGHT}>
                  {txt.nameTrimmed}
                </Text>
              )}
              {txt.datesTrimmed && (
                <Text {...commonTextProps} position={[0, txt.datesY, textZ]} fontSize={txt.datesSize} outlineWidth={txt.datesSize * 0.11} lineHeight={TEXT_LINE_HEIGHT}>
                  {txt.datesTrimmed}
                </Text>
              )}
            </Suspense>
